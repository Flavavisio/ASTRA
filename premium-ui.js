...
function applyPremium(ok){
 ...
 if(bar){bar.className="premiumStateBar "+(ok?"premiumOn":"premiumFree");bar.textContent=ok?"✦ Astralis Premium ativo":"Astralis Free · 1 consulta diária";}
...
document.addEventListener("click",e=>{if(e.target.id==="premiumCheckout"){let cfg=window.ASTRA_PAYMENT;if(!cfg?.checkoutEndpoint){alert("O Astralis Premium está preparado. Falta apenas ligar o fornecedor de pagamentos.");return}cfg.checkout(window.ASTRA_SELECTED_PLAN).catch(err=>alert(err.message));}});
